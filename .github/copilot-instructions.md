...
└── test-build.html             # Built dist/ testing
...
dist/                           # ⚠️ AUTO-GENERATED - CI COMMITS THIS
├── turntable-viewer.esm.js     # ES Module bundle (CDN / bundlers)
└── turntable-viewer.umd.cjs    # UMD/CommonJS bundle
```
...
### Testing
- **Local build**: http://localhost:3000/tests/test-local.html
- **CDN version**: http://localhost:3000/tests/test-cdn.html
...
2. Commit and push source/version changes only (excluding `dist/`)
3. Wait for the Build and Commit Distribution Files workflow to commit updated `dist/` to `main`
4. Pull the updated `main`, then create and push the tag: `git tag vX.X.X-beta -m "Release message"`
5. GitHub Actions attaches `dist/turntable-viewer.esm.js` and `dist/turntable-viewer.umd.cjs` to the release
...
<script type="module" src="https://cdn.jsdelivr.net/gh/NegativeMind/web-turntable-viewer-widget@v0.2.7-beta/dist/turntable-viewer.esm.js"></script>
...
- Display errors via `UIManager.showError()`
